...
function setLegend(title){
  const el = $("legend");
  const depthTxt = state.depthRange ? (lang==="fa" ? `عمق: ${state.depthRange}m` : `Depth: ${state.depthRange}m`) : "";
  const rollTxt = (state.rolling && state.rolling!=="instant") ? (lang==="fa" ? `میانگین: ${state.rolling.replace("mean","")} روز اخیر` : `Avg: last ${state.rolling.replace("mean","")} days`) : (lang==="fa" ? "لحظه‌ای" : "Instant");
  el.innerHTML = `
    <div style="font-weight:900; margin-bottom:6px">${title}</div>
    <div class="muted small" style="margin-bottom:6px">${depthTxt} • ${rollTxt}</div>
    <div class="bar"></div>
...
function setStatusHint(){
  ...
  $("playBtn").textContent = state.analyzed ? (state.playing ? "⏸ Pause" : "▶ Play") : "▶ Play";
  // Legend placeholder
  if(!state.analyzed){
    $("legend").innerHTML = `<div class="muted small" style="padding:10px">
      ${lang==="fa" ? "تنظیمات را انتخاب کنید و سپس روی «تحلیل» بزنید." : "Pick settings, then press Analyze."}
    </div>`;
  }
}
